      ...
      <aside className="sd">
        ...
        {Object.values(A).map(x=>(
          <button key={x.id} className={`ab ${ag===x.id?"on":""}`} onClick={()=>sag(x.id)}>
            <span className="ai">{x.icon}</span>
            <span style={{flex:1,minWidth:0}}><span className="an" style={{display:"block"}}>{x.name}</span><span className="ar">{x.role}</span></span>
            <span className="dot" style={{background:x.color}}/>
          </button>
        ))}
        <div className="mc">
          <div className="mt">🧠 Memory</div>
          {mem.slice(0,4).map((m,i)=><div key={i} className="mi"><span style={{color:"var(--ac)",fontSize:7}}>●</span><span>{m}</span></div>)}
        </div>
        <div className="ak">
          <div className="ad2"/>
          <div><div className="at">API Active</div><div className="av">{mk(key)}</div><button className="ck" onClick={()=>{setKey(null);sms([]);}}>Change</button></div>
        </div>
      </aside>
      <main className="mn">
        <div className="hd">
          <div style={{display:"flex",alignItems:"center",gap:11}}>
            <div className="hi" style={{background:`${agent.color}18`}}>{agent.icon}</div>
            <div><div className="hn">{agent.name}</div><div className="hr2">{agent.role}</div></div>
          </div>
          <div className="hx">
            <div className="st2"><span className="sd2"/>Online</div>
            {msgs.length>0&&<button className="cb" onClick={()=>sms([])}>Clear</button>}
          </div>
        </div>
        <div className="ms">
          {msgs.length===0?(
            <div className="wc">
              ...
              <div><div className="wt">Hello, I'm {agent.name}</div><div className="ws">Your {agent.role.toLowerCase()} — ready to help.</div></div>
              <div className="qg">
                {Q.map((q,i)=><button key={i} className="qb" onClick={()=>qq(q)}><span className="ql">{q.l}</span><span className="qs">{q.p}</span></button>)}
              </div>
            </div>
          ):msgs.map((m,i)=>(
            <div key={i} className={`mg ${m.role}`}>
              <div className="mv" style={m.role==="assistant"?{background:`${A[m.ag]?.color||"#00D4FF"}18`}:{}}>{m.role==="assistant"?(m.ai||"🦅"):"👤"}</div>
              <div>
                <div className="mb" dangerouslySetInnerHTML={m.role==="assistant"?{__html:md(m.content)}:undefined}>{m.role==="user"?m.content:undefined}</div>
                <div className="mm">{m.role==="assistant"?m.an:"You"} · {m.time}</div>
              </div>
            </div>
          ))}
          {load&&(
            <div className="mg assistant">
              <div className="mv" style={{background:`${agent.color}18`}}>{agent.icon}</div>
              <div><div className="mb"><div className="td2"><span/><span/><span/></div></div><div className="mm">{agent.name} thinking…</div></div>
            </div>
          )}
          <div ref={end}/>
        </div>
        <div className="ia">
          <div className="iw">
            <textarea className="if" placeholder={`Message ${agent.name}…`} value={inp} onChange={e=>si(e.target.value)} onKeyDown={kd} rows={1} onInput={e=>{e.target.style.height="auto";e.target.style.height=Math.min(e.target.scrollHeight,120)+"px";}}/>
            <button className="send" onClick={()=>send()} disabled={load||!inp.trim()}>↑</button>
          </div>
          <div className="ih">Enter to send · Switch agents anytime</div>
        </div>
      </main>
      <aside className="pn">
        <div className="sg">
          <div className="sc"><div className="sn2">{mc}</div><div className="sl2">Messages</div></div>
          <div className="sc"><div className="sn2">{tasks.filter(x=>x.d).length}/{tasks.length}</div><div className="sl2">Tasks</div></div>
          <div className="sc"><div className="sn2">{mem.length}</div><div className="sl2">Memories</div></div>
          <div className="sc"><div className="sn2">5</div><div className="sl2">Agents</div></div>
        </div>
        <div className="pts">
          {["tasks","mood"].map(x=><div key={x} className={`pt ${tab===x?"on":""}`} onClick={()=>stab(x)}>{x==="tasks"?"⚙️ Tasks":"💙 Mood"}</div>)}
        </div>
        {tab==="tasks"?(
          <>
            <div className="ph">Task Board</div>
            <div className="pb">
              {tasks.map(x=>(
                <div key={x.id} className={`ti ${x.d?"dn":""}`} onClick={()=>tg(x.id)}>
                  <div className={`tc ${x.d?"ch":""}`}>{x.d&&<span style={{color:"var(--bg)",fontSize:9}}>✓</span>}</div>
                  <div style={{flex:1}}><div className="tx2">{x.t}</div><span className={`tp ${x.p==="high"?"ph2":x.p==="mid"?"pm":"pl"}`}>{x.p}</span></div>
                </div>
              ))}
            </div>
            <div className="ta">
              <input className="tin" placeholder="Add task…" value={nt} onChange={e=>snt(e.target.value)} onKeyDown={e=>e.key==="Enter"&&at()}/>
              <button className="tab2" onClick={at}>+</button>
            </div>
          </>
        ):(
          <>
            <div className="ph">Mood Tracker</div>
            <div className="mtr">
              <div style={{fontSize:10,fontFamily:"var(--mo)",color:"var(--tf)"}}>How are you feeling?</div>
              <div className="msel">{M.map(m=><div key={m.n} className="mo2" onClick={()=>lm(m)}>{m.e} {m.n}</div>)}</div>
              {mood.length>0&&<>
                <div style={{fontSize:9,fontFamily:"var(--mo)",color:"var(--tf)",marginTop:6}}>RECENT</div>
                {mood.map((m,i)=>(
                  <div key={i} className="me">
                    <div style={{fontSize:18}}>{m.e}</div>
                    <div style={{flex:1}}><div style={{fontSize:11,fontWeight:600}}>{m.n}</div><div style={{fontSize:9,fontFamily:"var(--mo)",color:"var(--tf)"}}>{m.time}</div><div style={{height:3,background:"var(--bo)",borderRadius:2,marginTop:3}}><div style={{height:"100%",width:`${m.v}%`,background:m.c,borderRadius:2}}/></div></div>
                  </div>
                ))}
              </>}
            </div>
